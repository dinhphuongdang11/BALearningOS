...
export default function StageDetailView({
  ...
  return (
    <div className="space-y-6 max-w-5xl mx-auto font-sans" id="stage-detail-wrapper">
      {/* Back button */}
      <button
        onClick={onBack}
      ...
      {/* Hero Header Area */}
      <div className="bg-gradient-to-br from-slate-900 via-slate-900 to-slate-950 text-white rounded-xl border border-slate-800 p-6 md:p-8 shadow-md">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
          <div className="space-y-3 flex-1">
            <span className="text-xs uppercase bg-emerald-500/20 text-emerald-400 px-2.5 py-0.5 rounded border border-emerald-500/10 font-extrabold tracking-wider animate-pulse">
              Chi tiết Lộ trình
            </span>
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 pt-4">
        <div>
          ...
          <p className="text-xs text-slate-400 font-medium">Tự quản lý bài học, đọc tài liệu và lưu trữ ghi chép thực hành</p>
        </div>
        <button
          onClick={() => onAddLessonToStage(stage.id)}
          id="btn-add-lesson-stage-direct"
          className="flex items-center justify-center space-x-1.5 py-2 px-4 bg-emerald-600 hover:bg-emerald-700 text-white rounded-lg text-xs font-bold shadow-sm shadow-emerald-600/10 transition cursor-pointer"
        >
      ...
      <div className="space-y-4" id="lessons-cards-list">
        {totalCount === 0 ? (
          <div className="border border-dashed border-slate-800 bg-slate-900 p-12 text-center rounded-xl space-y-3">
            ...
            <h4 className="text-sm font-semibold text-slate-400">Chưa có bài học nào được định nghĩa</h4>
          ...
          stage.lessons
            ...
            .map((lesson) => (
              <div
                ...
                className="bg-slate-900 border border-slate-800 rounded-xl p-5 hover:border-slate-700 transition shadow-sm hover:shadow-md flex flex-col md:flex-row md:items-start justify-between gap-4"
              >
                {/* Text Content */}
                ...
                <div className="shrink-0 flex items-center space-x-2 w-fit">
                  <button
                    ...
                    className="flex items-center space-x-1.5 py-2 px-4 bg-slate-800 text-emerald-400 hover:bg-emerald-600 hover:text-white rounded-lg text-xs font-bold transition cursor-pointer"
                  >
