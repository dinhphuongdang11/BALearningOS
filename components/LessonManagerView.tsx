...
export default function LessonManagerView({
  ...
  return (
    <div className="space-y-6 max-w-6xl mx-auto font-sans text-slate-100" id="lesson-manager-view-wrapper">
      {/* Header index panel */}
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h2 className="text-2xl font-bold text-slate-100 tracking-tight">Thiết lập học liệu và Tiến độ</h2>
      ...
      {/* Query Filter row */}
      <div className="bg-slate-900 p-4 rounded-xl border border-slate-800 shadow-sm flex flex-col md:flex-row gap-4 items-center justify-between" id="filter-bar">
      ...
      {/* Data Table */}
      <div className="bg-slate-900 border border-slate-800 rounded-xl shadow-sm overflow-hidden" id="lesson-database-grid">
        {filteredLessons.length === 0 ? (
          <div className="p-16 text-center space-y-3">
            <AlertCircle className="w-10 h-10 text-slate-600 mx-auto stroke-1" />
            <h4 className="text-sm font-semibold text-slate-400">Không tìm thấy bài học nào</h4>
            ...
            <table className="w-full text-left border-collapse">
              ...
              <tbody className="divide-y divide-slate-800/60 text-xs">
                {filteredLessons.map((lesson) => (
                  <tr key={lesson.id} className="hover:bg-slate-800/20 transition">
                    <td className="py-4 px-6 font-bold text-slate-500 w-16">
                      #{lesson.order}
                    </td>
                    <td className="py-4 px-3 font-semibold text-slate-250 max-w-xs truncate">
                      {lesson.title}
                    </td>
                    ...
                    <td className="py-4 px-6 text-right space-x-1 w-44">
                      <button
                        ...
                        className="p-1.5 text-slate-500 hover:text-emerald-400 rounded-lg transition inline-flex items-center cursor-pointer"
                        title="Vào học / Xem"
                      >
